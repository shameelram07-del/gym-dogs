...
- Current look is **SLATE**: cool slate background `#080B11`, steel accent `#6C8CB8`, ice highlight
  ...
  migration. **Any raw `#FF2E93` or `rgba(255,46,147,…)` you find is a leftover, not a decision.**
  The token is `--steel`; if you see `--mag`, it predates the rename.
- **Use the CSS variables from `globals.css`** — `--bg --card --ink --accent --grad --ember --ice
  --steel` and friends. Never hardcode a hex in a component.
...
`src/lib/nutrition.js` is **pure** — no React, no fetch — and takes the clock as an argument so it
*can* be tested. **There is no test runner in this repo** (no jest, no vitest, no test script, no
test files) — earlier notes claiming it "is unit-tested" meant ad-hoc scratch scripts, not a
committed suite. Keep new logic pure and clock-injected anyway; it
