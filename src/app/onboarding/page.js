...
// All the steps and their options
const STEPS = [
  {
    id: 'gender',
...
export default function OnboardingPage() {
  ...
  const canProceed = () => {
    if (current.type === 'inputs') {
      return bodyInputs.weight && bodyInputs.height && bodyInputs.age;
    }
  ...
  const handleNext = () => {
    if (step < STEPS.length - 1) {
      ...
      // Save body stats into answers and finish
    ...
    return (
      <div className="min-h-screen bg-[#080C14] text-white flex flex-col items-center justify-center px-5 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center flex flex-col items-center gap-5">
          ...
          <div className="bg-white/4 border border-white/8 rounded-2xl p-5 w-full max-w-sm text-left space-y-3 mt-2">
            {Object.entries(answers).map(([key, val]) => (
              <div key={key} className="flex justify-between items-start gap-3">
                <p className="text-xs text-slate-500 tracking-widest uppercase">{key}</p>
                <p className="text-sm text-white font-semibold text-right">
                  {Array.isArray(val) ? val.join(', ') : typeof val === 'object' ? Object.entries(val).map(([k, v]) => `${v}${k === 'weight' ? 'kg' : k === 'height' ? 'cm' : 'yrs'}`).join(' · ') : val}
                </p>
              </div>
            ))}
          </div>
          <button className="w-full max-w-sm bg-gradient-to-r from-blue-500 to-violet-600 rounded-2xl py-4 text-white font-black tracking-widest text-sm uppercase shadow-lg shadow-blue-500/25">
            GO TO DASHBOARD →
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#080C14] text-white flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10 px-5 pt-14 pb-2">
        <div className="flex items-center justify-between mb-3">
          <button
            onClick={handleBack}
            className={`text-slate-500 text-sm tracking-wider ${step === 0 ? 'opacity-0 pointer-events-none' : ''}`}
          >
            ← BACK
          </button>
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center justify-center gap-2 rounded-2xl p-4 border transition-all duration-200 ${
                    selected
                      ? 'bg-gradient-to-br from-blue-500/25 to-violet-500/25 border-blue-500/60 shadow-lg shadow-blue-500/10'
                      : 'bg-white/4 border-white/8'
                  }`}
                >
                  <span className="text-2xl">{opt.icon}</span>
                  <span className={`text-xs font-bold tracking-wider text-center uppercase ${selected ? 'text-white' : 'text-slate-400'}`}>
                    {opt.label}
                  </span>
                  {selected && (
              ...
              <div key={field.key} className="bg-white/4 border border-white/8 rounded-2xl p-4">
                ...
                <div className="flex items-baseline gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setBodyInputs((prev) => ({ ...prev, [field.key]: e.target.value }))
                    }
